...
public partial class TwoWayDictionary<T1, T2> : IEnumerable<KeyValuePair<T1, T2>> where T1 : notnull
                                                                                  where T2 : notnull
{
    ...
    {
        if (dic1.TryGetValue(key, out T2? value))
        {
            dic1.Remove(key);
            dic2.Remove(value);
        }
    ...
    {
        if (dic2.TryGetValue(key, out T1? value))
        {
            dic2.Remove(key);
            dic1.Remove(value);
        }
